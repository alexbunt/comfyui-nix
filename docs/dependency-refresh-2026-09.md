...
- Current unstable nixpkgs removed Intel macOS support, so the supported matrix
  follows upstream and contains Linux plus Apple Silicon macOS.
...
- The launcher accepts Hugging Face Hub 1.x while preventing an unreviewed 2.x
  upgrade in the Manager-managed environment.
- The formatter wrapper preserves the documented no-argument `nix fmt` command.
