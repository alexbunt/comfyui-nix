    ...
    let
      platform =
        if pkgs.stdenv.hostPlatform.isLinux then
          ...
          (if pkgs.stdenv.hostPlatform.isAarch64 then "darwinArm64" else "darwinX86_64");
...
// lib.optionalAttrs (pkgs.stdenv.hostPlatform.isDarwin && pkgs.stdenv.hostPlatform.isx86_64) {
  pyarrow = final.buildPythonPackage {
    pname = "pyarrow";
    inherit (versions.vendored.pyarrowDarwinX86_64) version;
    format = "wheel";
    src = pkgs.fetchurl { inherit (versions.vendored.pyarrowDarwinX86_64) url hash; };
